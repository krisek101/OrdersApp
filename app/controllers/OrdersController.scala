...
class OrdersController @Inject()(db: Database, cc: ControllerComponents) extends AbstractController(cc) {
      ...
      jsonBody.map { json =>
        ...
        for (element <- elements) {
          ...
          if (productQuantity != 0) {
            ...
            statement.executeUpdate("UPDATE store SET quantity = quantity - 1 WHERE id='" + productID + "'")
          }
        }
      }
  ...
  def list = Action {
    ...
    try {
      ...
      Ok(views.html.orders(ordersList.toList))
    } finally {
      connection.close()
    }
  }

  def elements(id: Long) = Action {
    val connection = db.getConnection()
    try {
